package com.example.jihyun.myapplication1;
...
public class MainActivity extends ActionBarActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch(item.getItemId()) {
            ...
            case R.id.menu_camera:  // 카메라에서 캡처(섬네일)
                Intent cameraIntent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
                startActivityForResult(cameraIntent, REQUEST_CODE_CAMERA);
                break;

            case R.id.menu_camera_full: // 카메라에서 캠처(원본)
    ...
    void showExif(String path) {
        try {
            ...
            new AlertDialog.Builder(this).setTitle("EXIF 메타데이터").setMessage(msg).create().show();
